...
const createTask = async (task) => {
    try {
        const newTask = await db.one("INSERT INTO grid (title, priority,description,deadline, completed,category) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *",
            [task.title, task.priority, task.description, task.deadline, task.completed, task.category]
        );
        return newTask;
    } catch (err) {
        return err;
...
const updateTask = async (id, task) => {
    try {
        const updatedTask = await db.one(
            ...
            [task.title, task.priority, task.description, task.deadline, task.completed, task.category, id]
        );
        return updatedTask;
    } catch (err) {
        return err;
